...
def _get_best_intermediate_result_over_steps_multi(
    trial: "optuna.trial.FrozenTrial", directions: Sequence[StudyDirection]
) -> Sequence[float]:
    value_matrix = list(trial.intermediate_values_multi.values())
    best_results = []
    for i in range(len(directions)):
        values = [x[i] for x in value_matrix if not math.isnan(x[i])]
        if directions[i] == StudyDirection.MAXIMIZE:
            best_results.append(max(values))
        else:
            best_results.append(min(values))
    return best_results


def _get_percentile_intermediate_results_over_trials_multi(
    ...
    value_matrix = [
        t.intermediate_values_multi[step] for t in completed_trials if step in t.intermediate_values_multi
    ]

    if len(value_matrix) < n_min_trials:
        return math.nan
    ...
    for i in range(len(directions)):
        ...
        if directions[i] == StudyDirection.MAXIMIZE:
            _percentile = 100 - _percentile
        result.append(_min + (_max - _min) * (_percentile / 100))
...
class MultiObjectivePercentilePruner(BasePruner):
    ...
    def prune(self, study: "optuna.study.Study", trial: "optuna.trial.FrozenTrial") -> bool:
        ...
        best_intermediate_results = _get_best_intermediate_result_over_steps_multi(trial, directions)
        if any([math.isnan(x) for x in best_intermediate_results]):
            return True

        threshold_percentiles = _get_percentile_intermediate_results_over_trials_multi(
            completed_trials, directions, step, self._percentile, self._n_min_trials
        )
        if any([math.isnan(x) for x in threshold_percentiles]):
            return False
        
        # Prune only if all dimensions are worse than threshold
        for i in range(len(directions)):
            if directions[i] == StudyDirection.MAXIMIZE:
                if best_intermediate_results[i] >= threshold_percentiles[i]:
                    return False
            else:
                if best_intermediate_results[i] <= threshold_percentiles[i]:
                    return False
